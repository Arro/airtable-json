...
test("many widgets", async (t) => {
  ...
  const records_transformed = Array(100)
    .fill({})
    .map((widget, i) => {
      return {
        __id: `rec_${i}`,
...
test("many widgets populate", async (t) => {
  ...
  const w_compares = many_doodads.records.map(
    (doodad) => `RECORD_ID()+%3D+%27${doodad.id}%27`
  )
  ...
  for (const w of chunked_w_compares) {
    const the_correct_doodads = {
      records: many_doodads.records.slice(
        j * max_ids_per_query,
        (j + 1) * max_ids_per_query
      )
    }
  ...
  const records_transformed = Array(number_to_make)
    .fill({})
    .map((widget, i) => {
      return {
        __id: `rec_${i}`,
